...
class Player:
    ...
    def play(self, score: int, golf_map: sympy.Polygon, target: sympy.geometry.Point2D,
        ...
        # computed the middle points, tru to go the largest distance of the middle points
        if prev_admissible is None or prev_admissible or not self.remember_middle_points:
            ...
            # use memory to save computation intersection time
        ...
        # try to simulate the middle points from closest to target to the most far one
        midd_index = -1
            ...
            for _ in range(self.simulate_times):
                ...
                if not is_succ:
                    ...
                    if middle_failed_times[i] > self.tolerant_times:
                        middle_failed_times[i] = -1
                        break
                middle_failed_times[i] += final_point.distance(target)
        ...
        for i in distance_sorted_indexes:
            ...
            angle = sympy.atan2(middle_point.y - curr_loc.y, middle_point.x - curr_loc.x)
            for _ in range(self.simulate_times):
                is_succ, final_point = self.simulate_once(mid_to_target_distance[i], angle, curr_loc, golf_map)
                if not is_succ:
                    middle_failed_times[i] += 1
                    if middle_failed_times[i] > self.tolerant_times:
                        middle_failed_times[i] = -1
        ...
        desire_distance = mid_to_target_distance[midd_index]
        if midd_index != -1:
        ...
        # 4. if middle points are still not safe, choose the closest one to the target
